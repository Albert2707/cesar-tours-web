...
const Vehicle: FC<Props> = ({ setStep }) => {
  ...
  const { data, isLoading, isError } = useQuery(
    "vehicles",
    async () => {
      const res = await request.get(`vehicle/getVehicles/${passengerNo}/${bagsNo}`)
      return res.data
    },
  ...
  return (
    <div className="select-vehicle">
      <div className="booking-info">
        <div className="container-info">
          ...
          <div className="collection-time">
            ...
            <span>{format(departureDate!, idiom === "es" ? "dd/MM/yyyy" : "MM/dd/yyy")} - {departureHour}</span>
          </div>
      ...
      >
        {isError
          ? "Sometinhg went wrong"
          : isLoading ? <Loader /> : data.map((e: VehicleModel) => (
            <motion.div key={crypto.randomUUID()} className="vehicle" variants={variants}
            >
