...
const Login = () => {
    ...
    return (
        <motion.div className="login-container" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}>
            <Toaster />
            <div className="cesar-img">
                <img src="images/Cesar-logo.png" alt="Cesar logo" onLoad={() => setIsLoaded(true)} /> :
                {
                    !isLoaded &&
            ...
            <form action="" className="login-form" onSubmit={handleSubmit}>
                <input style={{ borderColor: errors.email ? "#e11d48" : "#f2f2f2" }} type="text" autoComplete="password" name="email" placeholder="Email" onChange={(e) => {
                    handleChange(e)
                    if (e.target.value) setErrors({ ...errors, email: false })
                }} />
                <div className="password-input" style={{ borderColor: errors.password ? "#e11d48" : "#f2f2f2" }}>
                    <input type={showPassword ? "text" : "password"} autoComplete="password" name="password" placeholder="Password" onChange={(e) => {
                        handleChange(e)
                        if (e.target.value) setErrors({ ...errors, password: false })
                    }} />
                    <button type="button" className="show-password" onClick={() => {
                ...
                <button className="login" type="submit">{loading ? <span className="loading"></span> : "Login"}</button>
            </form>
